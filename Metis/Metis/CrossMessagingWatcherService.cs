...
public class CrossMessagingWatcherService
{
    ...
    public List<byte[]> GetMessageHashes(TransactionReceipt receipt)
    {
        ...
        foreach (var sentMessageEvent in sentMessagesEvents) sentMessages.Add(Sha3Keccack.Current.CalculateHash(sentMessageEvent.Event.Message));

        return sentMessages;
    }


    public async Task<TransactionReceipt> GetCrossMessageMessageTransactionReceipt(Web3 web3layerToReceiveMessage, string messengerAddress,
        byte[] msgHash, CancellationToken token = default, int numberOfPastBlocks = 1000)
    {
